          ...
          {analytics
            ? statCards.map(({ label, value, icon: Icon, color, href, sub, alert }) => (
                <Link
                  key={label}
                  href={href}
                  className={`bg-white rounded-xl p-5 shadow-sm border transition-shadow hover:shadow-md group ${alert ? 'border-amber-300' : 'border-gray-100'}`}
                >
      ...
      {analytics && (
        <div className="grid lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            <h3 className="font-semibold text-gray-900 mb-1">Revenue Over Time</h3>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100">
            ...
            <div className="divide-y divide-gray-50">
              {analytics.topCustomers.length === 0 ? (
                ...
                <div key={tc.user.id} className="flex items-center justify-between px-6 py-3.5 hover:bg-gray-50 transition-colors">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-medium text-gray-900">{tc.user.name}</p>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100">
            ...
            <div className="divide-y divide-gray-50">
              {analytics.bestSellingProducts.length === 0 ? (
                ...
                <div key={bp.product.id} className="flex items-center justify-between px-6 py-3.5 hover:bg-gray-50 transition-colors">
                  <div className="flex items-center gap-3">
                    <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold text-white ${i === 0 ? 'bg-amber-400' : i === 1 ? 'bg-gray-400' : i === 2 ? 'bg-orange-400' : 'bg-gray-200 text-gray-600'}`}>
                      {i + 1}
                    </span>
                    <div>
                      <Link href={`/admin/products/${bp.product.id}`} className="text-sm font-medium text-gray-900 hover:text-blue-600">
                        {bp.product.name}
                      </Link>
                  ...
                  <div className="text-right">
                    <p className="text-sm font-semibold text-gray-800">{bp.totalSold} units</p>
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100">
            ...
            <div className="divide-y divide-gray-50">
              {analytics.mostViewed.length === 0 ? (
                ...
                <div key={mv.product.id} className="flex items-center justify-between px-6 py-3.5 hover:bg-gray-50 transition-colors">
                  <div className="flex items-center gap-3">
                    <span className="w-5 h-5 rounded text-xs font-semibold text-gray-400 flex items-center justify-center">
                      {i + 1}
                    </span>
                    <div>
                      <Link href={`/admin/products/${mv.product.id}`} className="text-sm font-medium text-gray-900 hover:text-blue-600">
                        {mv.product.name}
                      </Link>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100">
            <div className="flex items-center gap-2 px-6 py-4 border-b border-gray-100">
              ...
              <div>
                <h3 className="font-semibold text-gray-900">Most Wishlisted Products</h3>
            ...
            <div className="divide-y divide-gray-50">
              {analytics.mostWishlisted.length === 0 ? (
                ...
                <div key={wl.product.id} className="flex items-center justify-between px-6 py-3.5 hover:bg-gray-50 transition-colors">
                  <div className="flex items-center gap-3">
                    <span className="w-5 h-5 rounded text-xs font-semibold text-gray-400 flex items-center justify-center">
                      {i + 1}
                    </span>
                    <div>
                      <Link href={`/admin/products/${wl.product.id}`} className="text-sm font-medium text-gray-900 hover:text-blue-600">
                        {wl.product.name}
                      </Link>
